...
hashmap-gen-rename:
	...
	docker run --rm -it \
		...
		mkdir -p /go/src/$(codegen_package) && cp /out/map_gen.go /go/src/$(codegen_package)/map_gen.go && \
		...
		bash -c '! test -f /out/new_map_gen.go || echo 'type $(key_type) interface{}' >> /go/src/$(codegen_package)/types.go' && \
